...
export default function VirementPage({ navigate }) {
  ...
  const validate = () => {
    ...
    if (!form.iban.trim()) newErrors.iban = "IBAN requis";
    if (!form.bic.trim()) newErrors.bic = "BIC requis";
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      const newTransaction = {
        ...
        libelle: `Virement vers ${form.prenom} ${form.nom}`,
        montant: -montant,
        type: "debit",
        categorie: "Virement",
      };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <main className="max-w-lg mx-auto w-full px-4 py-16 flex flex-col items-center text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Virement effectué !</h2>
          <p className="text-gray-500 text-sm mb-2">
            {formatMontant(Number(form.montant))} {currentUser?.devise} envoyés à
          </p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col pb-10">
      ...
      <main className="max-w-lg mx-auto w-full px-4 py-6 space-y-5">
        <h1 className="text-xl font-bold text-gray-900">Effectuer un virement</h1>

        <div className="bg-red-600 rounded-2xl p-4 text-white">
          <p className="text-red-100 text-xs mb-1">Solde disponible</p>
          <p className="text-2xl font-bold">{formatMontant(currentUser?.solde)} {currentUser?.devise}</p>
        </div>

        <div className="bg-white rounded-2xl shadow-sm p-5 space-y-4">
          <h2 className="font-bold text-gray-800">Bénéficiaire</h2>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 space-y-4">
          <h2 className="font-bold text-gray-800">Montant</h2>
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1">Somme à virer</label>
        ...
        >
          {loading ? "Traitement en cours..." : (
            <><Send size={18} /> Envoyer le virement</>
          )}
        </button>
      </main>
